...
# --- دالة توليد الـ KML بعد تعديلها لتستقبل الإحداثيات مباشرة ---
def generate_kml(lines, spacing, length, lat, lon):
...
"""
    for i in range(lines):
        x_coord = i * spacing
        lon1 = lon + (x_coord / 111000)
        lat1 = lat
        lon2 = lon + (x_coord / 111000)
        lat2 = lat + (length / 111000)
...
# --- قسم التصدير الرئيسي (خارج الدالة ليعمل أوتوماتيك) ---
st.header("💾 تصدير خطة الطيران")

st.markdown("### 🌍 تحديد موقع منطقة الدراسة جغرافياً")
user_lat = st.number_input("دائرة العرض (Latitude) - المنصورة كمثال:", value=31.04, format="%.6f")
user_lon = st.number_input("خط الطول (Longitude) - المنصورة كمثال:", value=31.38, format="%.6f")

# استدعاء الدالة وتوليد البيانات بناءً على مدخلات المستخدم الحالية
kml_data = generate_kml(num_lines, line_spacing, area_length, user_lat, user_lon)

# ظهور زرار التحميل بشكل سليم
st.download_button(
